...
When it comes to data handling, `doBy` is nowhere nearly as powerful as more contemporary packages, e.g. those in the `tidyverse` eco system, [@tidyverse]. On the other hand, it can be hypothesized that the data handling functions in `doBy` remain appealing to a group of users because of their simplicity in use. In additon, `doBy` is based on classical data structures that are unlikely to undergo sudden changes. API....
...
L <- LE_matrix(tooth.gam, effect="dose")
linest(tooth.gam, L)
...
z<-coef(LSmeans(tooth.gam, effect="dose", type="link"))

mu <- g$linkinv(z$estimate)

se.mu <- sqrt(abs(g$mu.eta(z$estimate))) * z$std.error
...
because data is balanced). 
__FIXME: PAS PÅ HER.__
In general the estimates are will be 
...
installed). __FIXME: ref til pbkrtest...__
Unadjusted degrees of freedom are obtained by setting \verb|adjust.df=FALSE|. 
```{r}
