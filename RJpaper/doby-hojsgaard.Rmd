...
The `doBy` package (which first appeared on CRAN in 2006)
...
When it comes to data handling, `doBy` is nowhere nearly as powerful as more contemporary packages, such as those in the `tidyverse` eco system, [@tidyverse].  The `aggregate` function in base `R` provides functionality similar to `doBy`s `summaryBy` function. Another package to be mentioned in this connection is  `data.table`, @data.table. On the other hand, `doBy` is based on classical data structures that are unlikely to undergo sudden changes. Moreover, it can be hypothesized that the data handling functions in `doBy` remain appealing to a group of users because of their simplicity.
...
### Spcifications as formulas and lists
...
Covariates are fixed at their average value unles the `at=...`-argument is used. For example, 
...
`conc` - not the average of the squared values of
`conc`. Likewise `log(conc)` is the log of the average of `conc` - not the average of the log of `conc`, cfr. this:
```{r}
...
If we want to evaluate the LS--means at `conc=700` then we can do:
```{r}
...
The functions èsticon`, `linest`, `LSmeans` etc. are available for a range of model classes. We illustrate a few below:
We may decide to treat \verb|supp| as a random effect. This leads to a _linear mixed effects model_ as implemented in @lme4:
```{r}
library(lme4)
...
Notice here that the parameter estimates themselves are similar to those of a linear model (had data been completely balanced, the estimates would have been identical). However, the standard errors of the the estimates are much larger under the mixed model. This is due to `supp` being treated as a random effect.
...
We can also calculate LS--means for e.g.\ a Poisson or a gamma model in a _generalized linear model_ setting. Notice that the LS--means are on the scale of the linear predictor - not on the scale of the response.
...
Likewise, for _generalized estimating equations_ as implemented in the `geepack` package, [@geepack], we get:
```{r}
library(geepack)
