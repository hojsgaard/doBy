#' @title specialize
#' @description Fix certain arguments of a function call thereby
#'   creating a new function defined on a smaller domain (hence the
#'   name specialize).
#' @name specialize
#'
#' @param FUN Function
#' @param arglist List of the form name=value
#' @param envir Environment
#' 
#' @details The `curry::partial` function does the same as `specialize`, except that an error is produced 
...
#' @export
specialize <- function(FUN, arglist, envir=parent.frame()){
  ...
  as.function(c(gg, expr2), envir=envir)
}
