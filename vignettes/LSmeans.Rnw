...
library(knitr)
opts_chunk$set(
tidy=FALSE,fig.path='figures/LSmeans', fig.height=4
)
@
...
The interaction plot suggests an interaction, which turns out to be
...
The estimated length for each dose of orange juice (OJ) can be found as
<< >>= 
L <- matrix(c(1, 0, 0, 0, 
              1, 1, 0, 0,
...
maintained but remains in \doby\ for historical reasons. We can do:
...
<< >>= 
L <- linest_matrix(tooth1, effect="dose", at=list(supp="OJ"))
L
@
...
For a model with interactions, the $L$ matrix becomes
<< >>= 
...
<<>>=
linest_matrix(chick, effect="Diet")
@ %def
...
<<>>=
K1 <- linest_matrix(chick, effect="Diet", at=list(Time=1)); K1
@ %def
...
<<>>=
K <- linest_matrix(mod.nst, effect="BB")
t(K)
linest(mod.nst, L=K)

K <- linest_matrix(mod.nst, effect="BB", at=list(CC=2))
t(K)
linest(mod.nst, L=K)
