...
$\EE(y|A,B,C)$.  Sometimes one is interested in quantities like
$\EE(y|A)$. This quantity can not formally be found unless $B$ and $C$
are random variables such that we may find $\EE(y|A)$ by integration.
...
<<>>=
W <- matrix(c(1, 0, 0, 1/3, 1/3, 1/2, 1,
              1, 1, 0, 1/3, 1/3, 1/2, 1,
              1, 0, 1, 1/3, 1/3, 1/2, 1), nr=3, byrow=TRUE)
@ %def
...
<<>>=
CO2.ubal <- CO2.bal[-c(1, 5, 12, 16, 17, 18, 19, 20, 28, 34, 35, 36),]
CO2.ubal %>% head
...
Consider this task: Estimate the value of the response \tt uptake when
\tt conc=100 for each combination of \tt Type and \tt Treatment . This
...
uu1 <- cbind(pred=predict(fm1.ubal, newdata=grid1), grid1)
uu1
@ 

\paragraph{Common task 2:}
Estimate the value of the response \tt uptake when \tt
conc=100 for each level of \tt Treatment.  Formally this question can
...
There are different workarounds for this situation:

\begin{enumerate}
\item Compute the average of the predictions
  above. That corresponds to integrating \tt Type out from the
  expectation assuming that this factor has a uniform distribution in
  data.
\item Fit a model without \tt Type effect and
  repeat the steps above.
\end{enumerate}

<<>>=
## 1. Average of predictions
aggregate(pred ~ Treatment, data=uu1, FUN=mean)

## 2. Fit model without Type
fm2.ubal <- update(fm1.ubal, .~. - Type)
grid2 <- expand.grid(
    Treatment = c("nonchilled", "chilled"),
    conc      = 100
)
uu2 <- cbind(pred=predict(fm2.ubal, newdata=grid2), grid2)
uu2
@

Notice that the results are not identical. However, if data had been
balanced, then the approaches would have given the same results. The
first result is what is sometimes called a \emph{marginal mean} or a
\emph{population mean}, \emph{least squares mean} or \emph{LS mean}.

\paragraph{Common task 3:}
Estimate the value of the response \tt uptake for each level of \tt
Treatment. For the same reasons as above, this question can formally
not be answered under the model.  There are different workarounds for
this situation:

\begin{enumerate}
\item Compute the average of the predictions
  above, but with \tt conc fixed at its average.
\item Fit a model without \tt Type and \tt conc effect and
  repeat the steps above.
\end{enumerate}
...
## 1. Average of predictions
grid3 <- grid1
grid3$conc <- mean(CO2.ubal$conc)
uu3 <- cbind(pred=predict(fm1.ubal, newdata=grid3), grid3)

aggregate(pred ~ Treatment, data=uu3, FUN=mean)

## 2. Fit model without Type and conc
fm3.ubal <- update(fm1.ubal, .~. - Type - conc)
grid3 <- expand.grid(
    Treatment = c("nonchilled", "chilled")
)
uu3 <- cbind(pred=predict(fm3.ubal, newdata=grid3), grid3)
uu3
@

Notice again that the results are not identical. However, if data had been
balanced, then the approaches would have given the same results. The
first result is what is sometimes called a \emph{marginal mean} or a
\emph{population mean}, \emph{least squares mean} or \emph{LS mean}.
...
<<>>=
co2.lm1 <- lm(uptake ~ conc + Type + Treatment, data=CO2.ubal)
LSmeans(co2.lm1, effect="Treatment")
@ %def

If we use \code{log(conc)} instead we will get an error when
calculating LS--means:
<<eval=T>>=
co2.lm <- lm(uptake ~ log(conc) + Type + Treatment, data=CO2.ubal)
try(LSmeans(co2.lm, effect="Treatment"))
@ %def

In this case one can do
<<>>=
co2.lm2 <- lm(uptake ~ log.conc + Type + Treatment,
             data=transform(CO2.ubal, log.conc=log(conc)))
LSmeans(co2.lm2, effect="Treatment")
@ %def
...
<<>>=
co2.lm3 <- lm(uptake ~ conc + I(conc^2) + Type + Treatment, data=CO2.ubal)
LSmeans(co2.lm3, effect="Treatment")
@ %def
...
<<>>=
co2.lm4 <- lm(uptake ~ conc + conc2 + Type + Treatment, data=
              transform(CO2.ubal, conc2=conc^2))
LSmeans(co2.lm4, effect="Treatment")
@ %def
...
LSmeans(fm.glm, effect=~Treatment, type="response") ## IS THIS A BUG???
@ %def
...
Notice here that the estimates themselves identical to those of a
 linear model (that is not generally the case, but it is so here because data is balanced). In general the estimates are will be very similar but the standard errors are much larger under the mixed model.  This comes from that there that \code{supp} is treated as a random effect.
...
Notice that the degrees of freedom by default are adjusted using a
 Kenward--Roger approximation (provided that \pkg{pbkrtest} is installed). Unadjusted degrees of freedom are obtained by setting \verb|adjust.df=FALSE|.
...
LSmeans(fm.gee, effect=~Treatment, type="response") ## BUG??
@ %def
